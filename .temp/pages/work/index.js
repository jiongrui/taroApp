...
export default class Index extends Component {
  ...
  render() {
    return <View className="work">
        <ScrollView scrollY scrollTop={this.state.scrollTop} className="list">
          {this.state.works.map((work, midx) => <View className="list-item" key={`work${midx}`}>
              <View className="img-list">
                {work.list.map((item, nidx) => <View className="img-box" key={`work${nidx}`} style={{
              width: work.list.length < 3 ? `calc(${100 / work.list.length}% - 5px)` : `calc(${33.333333333333336}% - 5px)`,
              marginLeft: "5px",
              marginTop: nidx > 2 ? "5px" : 0
            }}>
                    <View className="img-inbox" onClick={this.handleShowPopup} data-midx={midx} data-nidx={nidx} id={`work${midx}${nidx}`} style={{ backgroundImage: `url(${item})` }}>
        ...
        {this.state.showPopup && <Popup list={this.state.workItem} onHandleHidePopup={this.handleHidePopup} onSettingScrollTop={this.handleSettingScrollTop} curId={this.state.curIndex} popupStyle={this.state.popupStyle} rect={this.state.rect} />}
      </View>;
  }
}